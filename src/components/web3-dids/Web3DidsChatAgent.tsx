...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/web3-dids-agent`;

export const Web3DidsChatAgent = () => {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      recordOperation({ agent: "federated", caseLabel: "Web3 y DIDs", question: text, tokensConsumed: tokensUsed });
    } catch {
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-medium">
                <Wallet className="w-2.5 h-2.5" /> Web3
              </span>
            </div>
