...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fundamentos-agent`;

export const FundamentosChatAgent = () => {
  const { t } = useTranslation("fundamentos");
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      recordOperation({ agent: "federated", caseLabel: "Fundamentos", question: text, tokensConsumed: tokensUsed });
    } catch {
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-medium">
                <Shield className="w-2.5 h-2.5" /> Fundamentos
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {suggestedQuestions.map((q) => (
                <button
                  ...
                  className="text-[11px] px-3 py-1.5 rounded-full border bg-background hover:bg-accent transition-colors text-foreground"
                >
                  {q}
                </button>
              ))}
          ...
          if (msg.role === "user") {
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex justify-end">
                <div className="max-w-[80%] px-3 py-2 rounded-xl bg-primary text-primary-foreground text-xs">
                  {msg.content}
                </div>
              </motion.div>
            );
        ...
        {guardWarning && (
          ...
          >
            <p className="text-[11px] text-destructive flex items-center gap-1.5">
              <AlertTriangle className="w-3 h-3" /> {guardWarning}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
